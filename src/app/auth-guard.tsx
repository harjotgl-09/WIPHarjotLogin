...
export function AuthGuard({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    console.log(`[AuthGuard] useEffect running. isLoading: ${isLoading}, user: ${user?.uid || 'null'}, pathname: ${pathname}`);
    // Wait until the authentication state is fully loaded
    if (isLoading) {
      console.log("[AuthGuard] Still loading auth state. Doing nothing.");
    ...
    if (!user && pathIsProtected) {
      console.log("[AuthGuard] User is not logged in and on a protected route. Redirecting to /login.");
